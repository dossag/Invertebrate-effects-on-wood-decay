...
We hypothesize that: (i) Besides climatic contributions to the invertebrate effect on decomposition, variation in deadwood traits will constrain invertebrate effects with different effects across climatic zones (Stronger magnitude in tropical zone) (ii) the effect of invertebrates will be greater in WD with a larger diameter; (iii) bark presence will increase  the effect of invertebrates on wood decomposition; (iv) the effects of invertebrates will be greater in natural (non-processed, either with or without bark) wood than in processed wood (e.g., blocks, stakes, dowels, mostly without bark) as the latter may be perceived as novel materials; (v) the effect of invertebrates on WD decomposition will be higher in Angiosperm than in gymnosperms, as the powerful anti-herbivory defenses in gymnosperms will deter invertebrate decomposers 
...
#Next we create a new dataframe (wood_meta_lab) with observations that report decomposition of roots

```{r, warning=FALSE, results='hide'}
wood_meta_root<- wood_meta[wood_meta$woody_debris_type == "root",]
dim(wood_meta_root)
```
we have 8 paired observations reporting the decomposition of roots
...
figure1
path<- getwd()
ggsave(filename="Geographical_distribution.png", plot=figure1, device="png",
...
```{r}
funnel(wood_meta_fauna$yi, wood_meta_fauna$vi, yaxis="seinv",
...
```{r}
ggsave(filename="diameter_overall.png", plot=plot4b, device="png",
       path=path, height=5, width=7, units="in", dpi=500)
```
...
fig_diameter
ggsave(filename="diameter.png", plot=fig_diameter, device="png",
       path=path, height=8, width=12, units="in", dpi=500)
```
...
figx
ggsave(filename="figx.png", plot=figx, device="png",
       path=path, height=8, width=12, units="in", dpi=500)
```
...
```
```{r,warning=FALSE}
ggsave(filename="diameter hist.png", plot=p_his, device="png",
...
```
```{r,warning=FALSE}
ggsave(filename="region_duration.png", plot=region_duration, device="png",
     path=path, height=4, width=5, units="in", dpi=500)
```
